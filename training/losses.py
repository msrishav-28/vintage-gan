...
class VGGPerceptualLoss(nn.Module):
    ...
    def __init__(
        ...
        # Use conv4_3 features. Pretrained weights are best for real training,
        # but can be disabled for offline smoke tests.
        if pretrained:
...
class ConsistencyLoss(nn.Module):
    ...
    def forward(
        self, generated: torch.Tensor, target_conditions: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if "multi" in self.detectors:
            detected_conditions = self.detectors["multi"](generated)
            return F.mse_loss(detected_conditions, target_conditions)
...
class VintageGANLoss(nn.Module):
    ...
    def __init__(
        ...
        self.perceptual_loss = VGGPerceptualLoss(pretrained=perceptual_pretrained)
