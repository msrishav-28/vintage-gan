...
def setup_training(config_path: str) -> Dict:
    """
...
def main():
    ...
    criterion = VintageGANLoss(
        perceptual_weight=config['training']['phase1_pretrain']['perceptual_loss_weight'],
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=config['training']['phase1_pretrain']['epochs']
    )
    ...
    for epoch in range(start_epoch, config['training']['phase1_pretrain']['epochs']):
        ...
        train_losses = train_epoch(
            generator, dataloaders['train'], criterion, optimizer,
            device, epoch, writer, config
        )
        
        # Validate
        val_losses = validate_epoch(
            generator, dataloaders['val'], criterion,
            device, epoch, writer, Path(config['paths']['output_dir'])
        )
        ...
        if (epoch + 1) % config['checkpointing']['save_every_n_epochs'] == 0:
            save_checkpoint(
                generator, optimizer, epoch, val_losses, config,
    ...
    # Final checkpoint
    save_checkpoint(
        generator, optimizer, config['training']['phase1_pretrain']['epochs'] - 1,
        val_losses, config, filename='generator_pretrain_final.pth'
    )
    
    print("\n" + "="*60)
    print("PRETRAINING COMPLETE")
    print(f"Best validation loss: {best_val_loss:.4f}")
    print(f"Final model saved: {config['paths']['checkpoint_dir']}/generator_pretrain_final.pth")
    print("="*60)
