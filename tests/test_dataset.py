...
class TestImageNetSubsetDataset:
    ...
    def test_dataset_initialization(self, dummy_dataset_dir):
        ...
        dataset = ImageNetSubsetDataset(
            root_dir=str(dummy_dataset_dir),
            split='train',
            image_size=512
        )
    ...
    def test_dataset_length(self, dummy_dataset_dir):
        ...
        assert len(train_dataset) == 10, f"Expected 10 train images, got {len(train_dataset)}"
    ...
    def test_image_value_range(self, dummy_dataset_dir):
        ...
        assert image.min() >= -1.5, f"Image min value {image.min()} below expected range"
    ...
    def test_get_sample_images(self, dummy_dataset_dir):
        ...
        assert all(isinstance(img, torch.Tensor) for img in samples), \
...
class TestVintageGANDataset:
    ...
    def test_output_shapes(self, dummy_clean_dataset, dummy_defect_generator):
        ...
        assert batch['clean'].shape == (3, 512, 512), \
            f"Clean shape incorrect: {batch['clean'].shape}"
        assert batch['defected'].shape == (3, 512, 512), \
            f"Defected shape incorrect: {batch['defected'].shape}"
        assert batch['condition'].shape == (6,), \
    ...
    def test_condition_value_range(self, dummy_clean_dataset, dummy_defect_generator):
        ...
        assert torch.all(condition >= 0.0), \
            f"Condition min {condition.min()} below 0"
        assert torch.all(condition <= 1.0), \
            f"Condition max {condition.max()} above 1"
    
    def test_reproducibility_with_seed(self, dummy_clean_dataset, dummy_defect_generator):
    ...
    def test_different_variants_have_different_conditions(
        ...
        for i in range(len(conditions)):
            for j in range(i + 1, len(conditions)):
                assert not torch.allclose(conditions[i], conditions[j], atol=1e-3), \
    ...
    def test_return_clean_false(self, dummy_clean_dataset, dummy_defect_generator):
        ...
        assert 'clean' not in batch, "Should not include 'clean' when return_clean=False"
...
"""
    ...
    def test_create_dataloaders_with_defect_generator(self, config_file):
        ...
        dataloaders = create_dataloaders(str(config_file), defect_generator=dummy_defect_gen)
        
        # Test loading a batch
        train_batch = next(iter(dataloaders['train']))
        
        assert isinstance(train_batch, dict), "Should return dictionary with VintageGAN dataset"
    ...
    def test_batch_shapes(self, config_file):
        ...
        batch_size = 2
        assert train_batch['clean'].shape == (batch_size, 3, 512, 512), \
            f"Clean batch shape incorrect: {train_batch['clean'].shape}"
        assert train_batch['defected'].shape == (batch_size, 3, 512, 512), \
            f"Defected batch shape incorrect: {train_batch['defected'].shape}"
        assert train_batch['condition'].shape == (batch_size, 6), \
...
class TestTensorConversion:
    ...
    def test_tensor_to_numpy_conversion(self, sample_dataset):
        ...
        assert 120 <= array[0, 0, 1] <= 135, f"G channel should be ~128, got {array[0, 0, 1]}"
    ...
    def test_numpy_to_tensor_conversion(self, sample_dataset):
        ...
        assert torch.abs(tensor[0, 0, 0] - (-1.0)) < 0.02, \
            f"R channel should be -1, got {tensor[0, 0, 0]}"
        assert torch.abs(tensor[1, 0, 0] - 0.0) < 0.02, \
            f"G channel should be 0, got {tensor[1, 0, 0]}"
        assert torch.abs(tensor[2, 0, 0] - 1.0) < 0.02, \
...
"""
    ...
    dataloaders = create_dataloaders(str(config_path), defect_generator=dummy_defect_gen)
