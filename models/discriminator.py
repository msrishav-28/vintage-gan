...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        conv5 = nn.Conv2d(base_filters * 8, 1, kernel_size=3, stride=1, padding=1)
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def __init__(self, num_scales: int = 3, condition_dim: int = 6):
        super().__init__()
        ...
        self.discriminators = nn.ModuleList(
            [Discriminator(condition_dim=condition_dim) for _ in range(num_scales)]
        )
    ...
    def forward(self, image: torch.Tensor, condition: torch.Tensor) -> list:
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"   ✓ Output patches: {out.shape[2]}×{out.shape[3]} = {out.shape[2]*out.shape[3]}"
        )
    except Exception as e:
        print(f"   ✗ Error: {e}")
        import traceback
