...
def calculate_fid(
    ...
    Target: FID < 50 (Specification Section 1.3)
    """
    try:
...
def calculate_ssim(
    ...
    Target: SSIM > 0.75 (Specification Section 1.3)
    """
    ssim_scores = []
...
def calculate_psnr(
    ...
    Target: PSNR > 22 dB (Specification Section 1.3)
    """
    psnr_scores = []
...
def calculate_condition_accuracy(
    ...
    if 'multi' in detectors:
        ...
        # Separate detectors
        detected_list = []
        for defect_name in ['grain', 'scratch', 'dust', 'vignette', 'color_shift', 'blur']:
            if defect_name in detectors:
                ...
                detected_list.append(test_conditions[:, len(detected_list):len(detected_list)+1])
...
def evaluate_model(
    ...
    print(f"SSIM: {ssim_score:.4f} (target: >0.75)")
    
    # PSNR
    print("\nCalculating PSNR...")
    psnr_score = calculate_psnr(generated_images, target_images)
    metrics['psnr'] = psnr_score
    print(f"PSNR: {psnr_score:.2f} dB (target: >22 dB)")
    ...
    if real_images_path is not None:
        ...
        for img_path in Path(real_images_path).glob('*.jpg'):
            img = np.array(Image.open(img_path).resize((512, 512)))
        ...
        if len(real_imgs) > 0:
            ...
            print(f"FID: {fid_score:.2f} (target: <50)")
        else:
    ...
    if detectors is not None:
        ...
        sample_clean = torch.from_numpy(target_images[idx]).permute(0, 3, 1, 2).float() / 127.5 - 1
